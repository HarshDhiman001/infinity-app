...
function StudentProfile({studentId,onBack,onFollow,followed}) {
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:200,background:"var(--bg)",overflowY:"auto",animation:"slideInRight .25s ease"}}>
      {/* Hero — position:relative wrapper so avatar never clips */}
      <div style={{position:"relative"}}>
        <div style={{height:120,overflow:"hidden",position:"relative"}}>
        ...
        {/* Avatar outside clipped hero */}
        <div style={{position:"absolute",bottom:-46,left:18,zIndex:10}}>
          <div style={{position:"relative",display:"inline-block"}}>
            <div style={{width:92,height:92,borderRadius:"50%",padding:3,background:`conic-gradient(${str?.color||"#1A4FD6"},#7C3AED,${str?.color||"#1A4FD6"})`}}>
              <img src={s.avatar} width={86} height={86} style={{borderRadius:"50%",display:"block",border:"3px solid var(--bg)",objectFit:"cover"}}/>
            </div>
      ...
      <div style={{maxWidth:600,margin:"0 auto",padding:"56px 18px 0"}}>
        ...
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:8}}>
          ...
          <FollowButton id={s.id} name={s.firstName||s.name.split(" ")[0]} isFollowed={isFollowed} onFollow={onFollow} followers={s.noteDownloads?Math.floor(s.noteDownloads/3):800}/>
        </div>
        ...
        <div style={{borderRadius:16,background:"linear-gradient(135deg,#0D0F1A,#1A0A2E)",padding:"14px 16px",marginBottom:14,border:"1px solid rgba(255,255,255,.08)"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6,background:"rgba(255,213,128,.1)",borderRadius:8,padding:"5px 10px",border:"1px solid rgba(255,213,128,.2)",display:"inline-flex"}}>
            <span style={{fontSize:11}}>🎯</span>
            ...
            {posts.map(p=>(
              <div key={p.id} style={{borderRadius:18,overflow:"hidden",marginBottom:10,background:"#fff",border:"1px solid var(--b1)",boxShadow:"0 2px 8px rgba(15,28,63,.05)"}}>
                ...
                <div style={{padding:"12px 14px"}}>
                  ...
                  <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:8}}>{p.tags.map(t=><span key={t} style={{fontSize:10,fontWeight:700,padding:"2px 8px",borderRadius:5,background:"rgba(26,79,214,.2)",color:"#93C5FD",border:"1px solid rgba(26,79,214,.25)",fontFamily:"'JetBrains Mono',monospace"}}>{t}</span>)}</div>
                  <div style={{display:"flex",gap:12,fontSize:11,color:"var(--muted)"}}>
                    <span>❤️ {p.likes>=1000?(p.likes/1000).toFixed(1)+"K":p.likes}</span><span>💬 {p.comments>=1000?(p.comments/1000).toFixed(1)+"K":p.comments}</span><span>🔖 {p.saves>=1000?(p.saves/1000).toFixed(1)+"K":p.saves}</span>
                    <span style={{marginLeft:"auto",fontFamily:"'JetBrains Mono',monospace",fontSize:10,fontWeight:700,color:"#93C5FD",background:"rgba(26,79,214,.15)",padding:"2px 8px",borderRadius:6}}>⚡{p.xp}XP</span>
                  </div>
                </div>
              </div>
            ))}
...
const handleGoogle=async()=>{
  ...
  if(screen==="splash") return (
    <div style={{minHeight:"100vh",background:"#0A0A0F",display:"flex",flexDirection:"column",position:"relative",overflow:"hidden"}}>
      {/* BG gradient */}
      <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse 80% 60% at 50% 30%,rgba(26,79,214,.25) 0%,transparent 70%)"}}/>
      <div style={{position:"absolute",inset:0,backgroundImage:"radial-gradient(circle,rgba(255,255,255,.04) 1px,transparent 1px)",backgroundSize:"32px 32px"}}/>
      {/* App name top */}
      <div style={{padding:"56px 28px 0",position:"relative",zIndex:1}}>
        <div style={{fontSize:13,color:"rgba(255,255,255,.35)",fontWeight:600,letterSpacing:".04em"}}>Infinity</div>
      </div>
      {/* Hero text */}
      <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"center",padding:"0 28px",position:"relative",zIndex:1}}>
        <div style={{fontSize:42,fontWeight:900,color:"#fff",lineHeight:1.1,letterSpacing:"-.04em",marginBottom:20}}>
          Study.<br/>Build.<br/>Win.
        </div>
        {/* Infinity logo */}
        <div style={{width:64,height:64,borderRadius:18,background:"linear-gradient(135deg,#1A4FD6,#7C3AED)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 8px 32px rgba(26,79,214,.4)"}}>
          <svg width="36" height="22" viewBox="0 0 60 34" fill="none"><path d="M30 17C30 17 20 4 10 4C2 4 2 30 10 30C20 30 30 17 30 17Z" stroke="white" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"/><path d="M30 17C30 17 40 4 50 4C58 4 58 30 50 30C40 30 30 17 30 17Z" stroke="white" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"/></svg>
        </div>
      </div>
      {/* Bottom bar */}
      <div style={{padding:"0 28px 52px",position:"relative",zIndex:1,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{width:36,height:36,borderRadius:"50%",border:"1px solid rgba(255,255,255,.15)",display:"flex",alignItems:"center",justifyContent:"center"}}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.4)" strokeWidth="2"><polyline points="15,18 9,12 15,6"/></svg>
        </div>
        <button onClick={()=>setScreen("login")} style={{width:56,height:56,borderRadius:"50%",background:"#fff",border:"none",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",boxShadow:"0 4px 20px rgba(255,255,255,.2)"}}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#0A0A0F" strokeWidth="2.5" strokeLinecap="round"><polygon points="5,3 19,12 5,21 5,3"/></svg>
        </button>
        <button onClick={()=>setScreen("login")} style={{background:"none",border:"none",color:"rgba(255,255,255,.5)",fontSize:13,fontWeight:600,cursor:"pointer",letterSpacing:".02em"}}>Start »»</button>
      </div>
    </div>
  );
  ...
  const Wrapper=({children})=>(
    <div style={{minHeight:"100vh",background:"#F0F4FB",display:"flex",flexDirection:"column"}}>
      {/* Header */}
      <div style={{padding:"52px 20px 0",display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
        <button onClick={()=>setScreen("splash")} style={{position:"absolute",left:20,width:34,height:34,borderRadius:"50%",background:"#fff",border:"1px solid #E0E8F4",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>
  ...
  const SocialRow=()=>(
    <div>
      <div style={{display:"flex",alignItems:"center",gap:10,margin:"20px 0"}}>
        <div style={{flex:1,height:1,background:"#EEF2FB"}}/><span style={{fontSize:12,color:"#aaa",fontWeight:500}}>Or Sign in with</span><div style={{flex:1,height:1,background:"#EEF2FB"}}/>
      </div>
      <div style={{display:"flex",justifyContent:"center",gap:20}}>
        {[
          <svg key="fb" width="22" height="22" viewBox="0 0 24 24" fill="#1877F3"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>,
          ...
          <svg key="ap" width="22" height="22" viewBox="0 0 24 24" fill="#000"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg>
        ].map((icon,i)=>(
          <button key={i} onClick={i===1?handleGoogle:onLogin} style={{width:52,height:52,borderRadius:"50%",background:"#F4F7FD",border:"1px solid #E4EAF6",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>
            {icon}
          </button>
        ))}
      </div>
  ...
  if(screen==="login") return (
    <Wrapper>
      <div style={{fontSize:22,fontWeight:900,color:"#1A1A2E",marginBottom:6,textAlign:"center"}}>Welcome to<br/>Infinity!</div>
      <div style={{fontSize:13,color:"#aaa",textAlign:"center",marginBottom:24}}>Sign in to continue your journey</div>
      ...
      <div style={{marginBottom:10}}>
        ...
        <div style={{position:"relative"}}>
          ...
          <button onClick={()=>setShowPw(v=>!v)} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#aaa",fontSize:16,padding:0}}>
            {showPw?"👁":"🙈"}
          </button>
        </div>
      </div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:22}}>
        <label style={{display:"flex",alignItems:"center",gap:6,fontSize:12,color:"#888",cursor:"pointer"}}>
          <input type="checkbox" style={{accentColor:"#1A4FD6"}}/> Remember me
        </label>
        <span style={{fontSize:12,color:"#1A4FD6",fontWeight:600,cursor:"pointer"}}>Forget password?</span>
      </div>
      <button onClick={onLogin} style={{width:"100%",padding:"14px",borderRadius:50,background:"linear-gradient(135deg,#1A4FD6,#3B5CE8)",border:"none",color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer",boxShadow:"0 4px 20px rgba(26,79,214,.35)",marginBottom:4}}>
        Login
      </button>
      {error&&<div style={{color:"#EF4444",fontSize:12,marginBottom:8,textAlign:"center",padding:"8px",background:"#FFF0F0",borderRadius:8}}>{error}</div>}
      <SocialRow/>
      <div style={{textAlign:"center",marginTop:16,fontSize:12,color:"#aaa"}}>
        Don't have an account? <span onClick={()=>setScreen("signup")} style={{color:"#1A4FD6",fontWeight:700,cursor:"pointer"}}>Sign up</span>
      </div>
    </Wrapper>
  );
  ...
  return (
    <Wrapper>
      <div style={{fontSize:22,fontWeight:900,color:"#1A1A2E",marginBottom:6,textAlign:"center"}}>Create an Account?</div>
      <div style={{fontSize:13,color:"#aaa",textAlign:"center",marginBottom:24}}>Join 12,000+ students on Infinity</div>
      <div style={{marginBottom:14}}>
      ...
      <label style={{display:"flex",alignItems:"center",gap:8,fontSize:12,color:"#888",marginBottom:22,cursor:"pointer"}}>
      ...
      <SocialRow/>
      <div style={{textAlign:"center",marginTop:16,fontSize:12,color:"#aaa"}}>
        Already have an account? <span onClick={()=>setScreen("login")} style={{color:"#1A4FD6",fontWeight:700,cursor:"pointer"}}>Sign in</span>
      </div>
    </Wrapper>
  );
}
